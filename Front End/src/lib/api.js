...
export const getGames = (topic, bool) => {
  ...
  return RawgApi.get(
    `/games?key=0c4bd7b594bf43a69030cea65b605923&&genres=${topic}&&ordering=-${RatingChange[bool]}`
  )
    .then(({ data }) => {
...
export const gamesSearch = (userInput) => {
  const myGamesArray = [];
  return RawgApi.get(
    `/games?search=${userInput}&&key=0c4bd7b594bf43a69030cea65b605923&&page_size=10`
  )
    .then(({ data }) => {
      const myResults = data.results;
      let userObject = {};
...
export const getGameById = (gameId) => {
  return RawgApi.get(
    `/games/${gameId}?key=0c4bd7b594bf43a69030cea65b605923`
  ).then(({ data }) => {
    let singleGameObj = {
      id: data.id,
      name: data.name,
